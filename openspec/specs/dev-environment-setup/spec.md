...
The Flutter SDK SHALL be installed on the developer's PATH at a stable 3.44.x
version (target: 3.44.2).
...
- THEN the output SHALL report Flutter `3.44.x` stable channel and the command
  SHALL exit with code 0

#### Scenario: Flutter doctor reports no blocking issues

- GIVEN Flutter SDK 3.44.x is on PATH and Android SDK is configured
